...
function getUserList(){
	...
	$.ajax({
		...
		success : function(data) {
			...
			$('#usermanage_Grid_div').append('<table id="usermanage_Grid_table"></table>');
			...
			grid.jqGrid({
				...
				datastr : dataList,
				colModel: [
				           {name:'userid', label:'사용자아이디', align:'center', width:'10%'},
				           {name:'username', label:'사용자명', align:'center', width:'10%'},
				           {name:'Phone', label:'전화번호', align:'center', width:'10%'},
				           {name:'Mobile', label:'핸드폰', align:'center', width:'10%'},
				           {name:'Email', label:'이메일', align:'center', width:'10%'},
				           {name:'Address', label:'주소', align:'center', width:'10%'},
				           {label : "Edit", name: 'Edit', sorttype: 'string', align: 'center', width: '8%',
				        	   formatter:function (cellvalue, options, rowObject) {    
					        	   return '<input type="button" class="btn btn-default" style="border:1px solid #ccc;color:white;background: linear-gradient( to bottom, white, #c1d4f1 );color: #2e4481;width:100%" onclick="editUserinfo('+options.rowId+')" value="Edit"/>'; 
					           }
				           },
				           {label : "Del", name: 'del', sorttype: 'string', align: 'center', width: '8%',
				        	   formatter:function (cellvalue, options, rowObject) {    
					        	   return '<input type="button" class="btn btn-default" style="border:1px solid #ccc;color:white;background-color:red;width:100%" onclick="deleteUserinfo('+options.rowId+')" value="Delete"/>'; 
					           }
				           }
				],
				autowidth : true,
...
function deleteUserinfo(rowId){
	...
	$('#delete_userinfo_Btn').attr("onclick" , "deleteUserinfoSave("+rowId+")");
}
